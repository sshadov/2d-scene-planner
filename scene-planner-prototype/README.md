...
В прототипе можно задать размеры помещения и сцены, затем добавить LED-экраны, поверхности, камеры, проекторы и световые приборы. Каждый объект получает стабильный `pluginId` и абсолютный `transform` Designer. Экран и проекционная поверхность хранят размер, разрешение, шаг пикселя и положение нижнего центра; проектор хранит положение линзы и точку `Look At`; камера создаётся как обычный `Camera`. Сетка имеет фиксированный шаг 1 м и показывает абсолютную плоскость `X/Z`; перетаскивание не меняет вертикальный `Y`. Планы v7 сохраняются локально, а «Экспорт JSON» создаёт переносимый файл.
