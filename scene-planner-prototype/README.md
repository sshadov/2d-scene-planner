...
Hosted startup treats the current Designer Stage as authoritative, blocks automatic LIVE until transport safety is accepted when needed, saves resources best-effort, and requires a complete scene import. The single Diagnostics panel shows Planner, Python API, and LIVE events in chronological order. Resolution is synchronized through the documented inherited `Display.resolution` property for LED Screen, DMX Screen, and Surface; Projector resolution is intentionally not a Planner control.
