...
В прототипе можно задать размеры помещения и сцены, затем добавить LED-экраны, поверхности, камеры, проекторы и световые приборы. Каждый объект получает стабильный `pluginId` и абсолютный `transform` Designer. LED-экран хранит размер, разрешение, PPI и положение нижнего центра; поверхность хранит размер и разрешение; проектор хранит положение линзы и видимую точку `Look At`, которую можно привязать к центру поверхности. Сетка имеет фиксированный шаг 1 м и показывает абсолютную плоскость `X/Z`; перетаскивание не меняет вертикальный `Y`. Планы v8 сохраняются локально, а «Экспорт JSON» создаёт переносимый файл.
