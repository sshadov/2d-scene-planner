...
В прототипе можно задать размеры помещения и добавить LED-экраны, поверхности, камеры, проекторы и световые приборы. Каждый объект получает стабильный `pluginId`, нативные Designer-координаты `X/Y/Z`, полный поворот `Rx/Ry/Rz` и вертикальную привязку. Помещение центрировано на `X=0, Z=0`; его границы идут от отрицательной до положительной половины размера. Сетка имеет фиксированный шаг 1 м и показывает плоскость `X/Z`; перетаскивание не меняет вертикальный `Y`. Планы сохраняются в локальном хранилище браузера, а «Экспорт JSON» сохраняет переносимый файл.
