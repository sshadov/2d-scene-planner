...
Hosted startup treats the current Designer Stage as authoritative, blocks automatic LIVE until transport safety is accepted when needed, saves resources best-effort, and requires a successful scene import. The single Diagnostics panel shows Planner, Python API, and LIVE events in chronological order. DMX Light removal from the Device list is not changed in version `0.22.0` and remains a separate validation task.
