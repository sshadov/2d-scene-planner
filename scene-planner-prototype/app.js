(() => {
  ...
  function clearPlannerScene() {
    ...
    if (!window.confirm("Очистить объекты в планировщике? Объекты, уже записанные в Designer, удалены не будут.")) return;
  ...
  function setupStaticInputs() {
    ...
    ["width", "depth", "height"].forEach(key => bindNumericInput(stageInputs[key], () => state.stage[key], value => { state.stage[key] = ["width", "depth"].includes(key) ? clamp(value, .5, 100) : clamp(value, 0, 20); }, .1));
  ...
  function setObjectHeight(object, value) { const height = finite(value, NaN); if (!Number.isFinite(height)) return false; object.transform.position.y = state.stage.measureFromStage ? stageTopY() + height : height; return true; }
