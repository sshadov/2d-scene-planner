(() => {
  ...
  const typeConfig = {
    ...
    designer: { label: "Designer Object", group: "Designer Objects", color: "#9aa7b4", radius: 0.3, defaultHeight: 0 }
  };
  const roomInputs = {
    width: document.querySelector("#room-width"), depth: document.querySelector("#room-depth")
  };
  const stageInputs = {
    enabled: document.querySelector("#stage-enabled"), width: document.querySelector("#stage-width"), depth: document.querySelector("#stage-depth"), height: document.querySelector("#stage-height"), measureFromStage: document.querySelector("#measure-from-stage")
  };
  ...
  const state = {
    room: { ...defaults.room }, stage: { ...defaults.stage, enabled: true }, objects: [], selectedId: null, selectedIds: new Set(), zoom: 1, liveEnabled: false,
  ...
  function syncStaticInputs() {
    ...
    [stageInputs.width, stageInputs.depth, stageInputs.height, stageInputs.measureFromStage].forEach(input => { if (input) input.disabled = !state.stage.enabled; });
  ...
  function fieldSections(object) {
    const heightLabel = state.stage.measureFromStage ? "Position Y (Scene)" : "Position Y"; const position = [["X", "transform.position.x", object.transform.position.x, .1, "m"], ["Z", "transform.position.z", object.transform.position.z, .1, "m"], [heightLabel, "transform.position.y", objectHeightValue(object), .1, "m"]];
  ...
  async function openSyncDialog() {
    ...
    message.textContent = diff.designerCount ? `Current scene contains ${diff.designerCount} object(s). Manual objects (${diff.preserve.length}) will remain unchanged.` : "Current scene is empty. Only objects from this plan will be created."; const warnings = [diff.standardCandidates.length ? `Recognized default objects: ${diff.standardCandidates.length}.` : "No Designer objects will be deleted automatically."]; if (diff.inspectionWarnings.length) warnings.push(`Skipped damaged references: ${diff.inspectionWarnings.length}.`); warning.textContent = warnings.join(" "); warning.hidden = false; confirm.disabled = adapter.capabilities?.liveUpdate !== true;
  }
  async function syncToDesigner(diff) {
    const records = state.sync.objects || {};
    if (typeof diff.adapter.syncEnvironment === "function") await diff.adapter.syncEnvironment({ room: state.room, stage: state.stage });
  ...
  async function runLiveSync() {
    ...
    try {
      ...
      if (diff.create.length || diff.update.length) await syncToDesigner(diff); else if (typeof adapter.syncEnvironment === "function") await adapter.syncEnvironment({ room: state.room, stage: state.stage });
      document.querySelector("#adapter-status").textContent = `LIVE: synced · ${new Date().toLocaleTimeString()}`;
    } catch (error) {
      state.sync.errors.live = error.message || String(error); persist(false); document.querySelector("#adapter-status").textContent = `LIVE: error · ${error.message || error}`;
    } finally {
  ...
  function importedObject(item, index) {
    ...
    const name = item.description || item.name || String(item.path || "").split(/[\\/]/).pop()?.replace(/\.apx$/i, "") || `${typeConfig[knownType].label} ${index + 1}`;
  ...
  async function importDesignerScene(adapter) {
    ...
    const imported = (inspection.objects || []).map(importedObject); const importedIds = new Set(imported.map(object => object.pluginId));
    const localOnly = state.objects.filter(object => !importedIds.has(object.pluginId));
    state.objects = [...imported, ...localOnly]; state.selectedId = state.objects[0]?.id ?? null; state.selectedIds = new Set(state.selectedId ? [state.selectedId] : []);
    ...
    state.sync.objects = { ...(state.sync.objects || {}), ...importedRecords };
    state.sync.lastSyncAt = new Date().toISOString(); persist(false); render();
  }
  ...
  document.querySelector("#clear-scene-button").addEventListener("click", clearPlannerScene); document.querySelector("#export-button").addEventListener("click", openSyncDialog); document.querySelector("#json-button").addEventListener("click", exportSceneJson); document.querySelector("#confirm-sync").addEventListener("click", confirmSync); document.querySelector("#delete-standards").addEventListener("click", deleteSelectedStandards); document.querySelectorAll("input[name=sync-mode]").forEach(input => input.addEventListener("change", () => { if (!document.querySelector("#sync-modal").hidden) openSyncDialog(); })); ["#close-sync", "#cancel-sync"].forEach(selector => document.querySelector(selector).addEventListener("click", () => { document.querySelector("#sync-modal").hidden = true; })); document.querySelector("#objects-toggle").addEventListener("click", () => { const rail = document.querySelector("#object-rail"); rail.hidden = !rail.hidden; document.querySelector("#objects-toggle").setAttribute("aria-expanded", String(!rail.hidden)); }); document.querySelector("#live-toggle").addEventListener("change", event => { const adapter = getAdapter(); if (event.target.checked && !adapter) { event.target.checked = false; state.liveEnabled = false; document.querySelector("#adapter-status").textContent = "LIVE unavailable · Designer API not connected"; } else { state.liveEnabled = event.target.checked; clearTimeout(liveSyncTimer); liveSyncTimer = null; persist(false); document.querySelector("#adapter-status").textContent = state.liveEnabled ? "LIVE active" : "LIVE off"; if (state.liveEnabled) scheduleLiveSync(); } document.querySelector("#export-button").disabled = state.liveEnabled; renderStatus(); });
  ...
  const finishStartup = async () => { if (adapter) { try { await importDesignerScene(adapter); adapterStatus.textContent = "Designer scene imported"; } catch (error) { adapterStatus.textContent = `Designer import failed · ${error.message || error}`; } } appReady = true; if (state.liveEnabled) { adapterStatus.textContent = "LIVE active"; scheduleLiveSync(); } renderStatus(); };
