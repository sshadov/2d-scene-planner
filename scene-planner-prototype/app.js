(() => {
  ...
  // Reserved for the official WebSocket Live Update adapter. HTTP polling is
  // intentionally absent so this control cannot masquerade as Live Update.
  ...
  document.querySelector("#clear-scene-button").addEventListener("click", clearPlannerScene); document.querySelector("#export-button").addEventListener("click", openSyncDialog); document.querySelector("#json-button").addEventListener("click", exportSceneJson); document.querySelector("#confirm-sync").addEventListener("click", confirmSync); document.querySelector("#delete-standards").addEventListener("click", deleteSelectedStandards); document.querySelectorAll("input[name=sync-mode]").forEach(input => input.addEventListener("change", () => { if (!document.querySelector("#sync-modal").hidden) openSyncDialog(); })); ["#close-sync", "#cancel-sync"].forEach(selector => document.querySelector(selector).addEventListener("click", () => { document.querySelector("#sync-modal").hidden = true; })); document.querySelector("#objects-toggle").addEventListener("click", () => { const rail = document.querySelector("#object-rail"); rail.hidden = !rail.hidden; document.querySelector("#objects-toggle").setAttribute("aria-expanded", String(!rail.hidden)); }); document.querySelector("#live-toggle").addEventListener("change", event => { const adapter = getAdapter(); if (event.target.checked && (!adapter || adapter.capabilities?.liveUpdate !== true)) { event.target.checked = false; state.liveEnabled = false; document.querySelector("#adapter-status").textContent = "LIVE unavailable · official WebSocket adapter is not connected"; } else { state.liveEnabled = event.target.checked; persist(false); document.querySelector("#adapter-status").textContent = state.liveEnabled ? "LIVE active" : "LIVE off"; if (state.liveEnabled) scheduleLiveSync(); } document.querySelector("#export-button").disabled = false; renderStatus(); });
