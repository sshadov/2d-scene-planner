(() => {
  ...
  async function openSyncDialog() {
    ...
    message.textContent = diff.designerCount ? `Current scene contains ${diff.designerCount} object(s). Manual objects (${diff.preserve.length}) will remain unchanged.` : "Current scene is empty. Only objects from this plan will be created."; const warnings = [diff.standardCandidates.length ? `Recognized default objects: ${diff.standardCandidates.length}.` : "No Designer objects will be deleted automatically."]; if (diff.missing.length) warnings.push(`Missing in Designer: ${diff.missing.length}; they will not be recreated automatically.`); if (diff.inspectionWarnings.length) warnings.push(`Skipped damaged references: ${diff.inspectionWarnings.length}.`); warning.textContent = warnings.join(" "); warning.hidden = false; confirm.disabled = adapter.capabilities?.liveUpdate !== true;
  }
  ...
  async function runLiveSync() {
    ...
    try {
      ...
      if (diff.create.length || diff.update.length || diff.adopt.length) await syncToDesigner(diff); else await syncEnvironmentIfChanged(adapter);
