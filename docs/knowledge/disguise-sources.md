...
Runtime validation against the installed 2026 build found two API details not captured accurately by static assumptions. `Resource.isBad`, `isIncomplete`, and `isInError` are boolean attributes at runtime although the supplied `d3.pyi` declares methods. Also, `Object.remove()` does not persistently remove top-level equipment from typed Stage lists; assign a filtered list through `stage.dmxLights`, `stage.cameras`, `stage.projectors`, and the corresponding other typed setter, save Stage, and only then remove package resources. LED/DMX/surface devices receive a separate `DirectProjection`; discover sole-screen references with `findResourcesPointingToThis(DirectProjection)` and remove them during owned-resource cleanup.

Ownership cannot be inferred later from a human-readable resource name. A create operation must return and persist the exact main, config, child, projection-object, and generated `DirectProjection` paths it created. Managed deletion must require that ownership record and intersect discovered auxiliaries with it. Imported/adopted Designer resources are unowned and must not be physically deleted by the Planner. If legacy sync data lacks the complete ownership list, refuse physical deletion rather than guessing. When a main resource is renamed, replace its old main path in the ownership list while leaving auxiliary paths unchanged.
