...
FixtureGroup/DMX Device-list deletion now follows the same explicit two-step contract as other Stage objects: detach from `stage.dmxLights`, save/read back the typed collection, then remove the exact main Resource when the confirmation requests Device List deletion. Auxiliary `DirectProjection` cleanup is optional and only occurs for an exact typed, named, single-target Planner-owned path; imported or shared dependencies are left untouched.
