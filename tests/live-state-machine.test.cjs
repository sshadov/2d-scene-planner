...
(async () => {
  ...
  const payload = { pluginId: "screen-1", type: "screen", name: "screen", transform: { position: { x: 1, y: 0, z: 2 }, rotation: { x: 0, y: 0, z: 0 } }, geometry: { width: 4, height: 2 }, media: { resolutionX: 1920, resolutionY: 1080 } };
  const projectorPayload = { pluginId: "projector-1", type: "projector", name: "projector", transform: { position: { x: -3, y: 3, z: -5 }, rotation: { x: 0, y: 0, z: 0 } }, lookAt: { x: 2, y: 0, z: 4 }, media: { resolutionX: 1920, resolutionY: 1080 } };
  ...
  const statuses = []; const receivedValues = [];
  const startPromise = adapter.liveStart({ onStatus(status) { statuses.push(status); }, onValuesChanged(change) { receivedValues.push(change); }, onSceneChanged() {} });
  ...
  const screenSubscriptions = first.sent.filter(item => item.subscribe?.object === "getByUID(0x10)").flatMap(item => item.subscribe.properties);
  ...
  assert.ok(screenSubscriptions.includes("object.resolution.x"));
  assert.ok(screenSubscriptions.includes("object.resolution.y"));
  assert.deepEqual(projectorSubscriptions, ["object.description", "object.configPosition", "object.configLookAt", "object.configThrowRatio", "object.fieldOfView", "object.configLookDistance", "object.configRotation.z", "object.resolution.x", "object.resolution.y"]);
  ...
  assert.ok(projectorSubscriptions.includes("object.resolution.x"));
  assert.ok(projectorSubscriptions.includes("object.resolution.y"));
  ...
  const resolutionXBinding = converged.find(binding => binding.field === "media.resolutionX");
  const resolutionYBinding = converged.find(binding => binding.field === "media.resolutionY");
  first.message({ valuesChanged: [{ id: resolutionXBinding.id, value: 3840 }, { id: resolutionYBinding.id, value: 2160 }] });
  assert.deepEqual(JSON.parse(JSON.stringify(receivedValues.slice(-2).map(change => ({ field: change.field, value: change.value })))), [{ field: "media.resolutionX", value: 3840 }, { field: "media.resolutionY", value: 2160 }]);
  const projectorResolutionXBinding = converged.find(binding => binding.pluginId === "projector-1" && binding.field === "media.resolutionX");
  const projectorResolutionYBinding = converged.find(binding => binding.pluginId === "projector-1" && binding.field === "media.resolutionY");
  first.message({ valuesChanged: [{ id: projectorResolutionXBinding.id, value: 1080 }, { id: projectorResolutionYBinding.id, value: 1920 }] });
  assert.deepEqual(JSON.parse(JSON.stringify(receivedValues.slice(-2).map(change => ({ pluginId: change.pluginId, field: change.field, value: change.value })))), [{ pluginId: "projector-1", field: "media.resolutionX", value: 1080 }, { pluginId: "projector-1", field: "media.resolutionY", value: 1920 }], "External Projector resolution changes must reach the Projector record");
