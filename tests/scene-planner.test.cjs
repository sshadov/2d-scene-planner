...
function createHarness(saved, storageKey = "disguise-scene-generator-state-v5", locationOptions = {}) {
  ...
  const updateMode = new ElementStub("update"); updateMode.checked = true; const cleanMode = new ElementStub("clean");
  const storage = new Map(Object.entries(locationOptions.storageEntries || {}).map(([key, value]) => [key, typeof value === "string" ? value : JSON.stringify(value)]));
...
(async () => {
  ...
  const offlineAdapter = {
    inspectScene: async () => { offlineCalls.push("inspect"); return { complete: true, objects: [], floorY: 0 }; },
    createObject: async () => { offlineCalls.push("create"); return {}; },
    updateObject: async () => { offlineCalls.push("update"); return {}; },
    deleteManagedObjects: async () => { offlineCalls.push("delete"); return {}; },
    activeTransportStatus: async () => { offlineCalls.push("transport"); return { known: true, running: false, transports: ["Stop"] }; },
    liveStart: async () => { offlineCalls.push("live"); },
    getLiveState: () => ({ socket: "open" }),
    liveSync: () => { offlineCalls.push("live-sync"); },
    getOperationLogs: () => { offlineCalls.push("api-logs"); return []; },
    getLiveLogs: () => { offlineCalls.push("live-logs"); return []; },
    setDiagnosticsListener: () => { offlineCalls.push("diagnostics"); }
  };
  const pluginStoredPlan = { version: 11, stage: { width: 99, depth: 88 }, objects: [sceneObject("camera", "plugin-plan")], sync: { objects: {} }, liveEnabled: true };
  const offlineStoredPlan = { version: 11, stage: { width: 24, depth: 14 }, objects: [sceneObject("surface", "offline-plan")], sync: { objects: {} }, liveEnabled: true };
  const offlineHarness = createHarness(undefined, "unused", {
    hostname: "offline-file",
    port: "",
    mode: "offline",
    adapter: offlineAdapter,
    storageEntries: {
      "disguise-scene-generator-state-v11": pluginStoredPlan,
      "2d-scene-planner-offline-state-v11": offlineStoredPlan
    }
  });
  await offlineHarness.runStartupGate();
  await new Promise(resolve => setTimeout(resolve, 0));
  assert.deepEqual(JSON.parse(JSON.stringify(offlineHarness.state.stage)), { width: 24, depth: 14 }, "Offline mode must restore only its isolated local plan");
  assert.deepEqual(JSON.parse(JSON.stringify(offlineHarness.state.objects.map(item => item.pluginId))), ["offline-plan"]);
  assert.equal(offlineHarness.state.liveEnabled, false);
  offlineHarness.addObjectAt("screen", 2, 3);
  await new Promise(resolve => setTimeout(resolve, 0));
  assert.deepEqual(offlineCalls, [], "Offline mode must never cross a Designer integration boundary");
  const offlineSurface = offlineHarness.newObject("surface", 0, 0); offlineSurface.geometry = { width: 2, height: 4 };
  const offlineProjector = offlineHarness.newObject("projector", 0, -12); offlineProjector.transform.position.y = 2; offlineProjector.lookAt = { x: 0, y: 2, z: 0 }; offlineProjector.targetSurfacePluginId = offlineSurface.pluginId;
  offlineHarness.state.objects.push(offlineSurface, offlineProjector);
  const offlineBinding = await offlineHarness.commitProjectorBinding(offlineProjector);
  assert.deepEqual(JSON.parse(JSON.stringify(offlineBinding)), { offline: true });
  assert.equal(offlineProjector.optics.throwRatio, 3, "Offline Surface binding must run the shared Auto Throw Ratio calculation");
  offlineHarness.queueProjectorProjection(offlineProjector, "offline-test");
  await new Promise(resolve => setTimeout(resolve, 550));
  assert.equal(offlineProjector.projectorRoll, 90, "Offline final projection update must apply the shared portrait roll rule");
  const persistedOffline = JSON.parse(offlineHarness.__storage.get("2d-scene-planner-offline-state-v11"));
  assert.equal(persistedOffline.objects.some(item => item.pluginId === offlineProjector.pluginId), true);
  assert.equal(offlineHarness.__storage.get("disguise-scene-generator-state-v11"), JSON.stringify(pluginStoredPlan), "Offline writes must not touch plugin persistence");
  assert.deepEqual(offlineCalls, [], "Offline projector work must remain local-only");
  ...
  assert.match(indexSource, /title>2D Scene Planner v0\.24\.0/);
