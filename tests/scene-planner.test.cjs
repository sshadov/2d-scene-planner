...
(async () => {
  ...
  assert.match(indexSource, /id="stage-height"/); assert.match(indexSource, /id="measure-from-stage"/); assert.doesNotMatch(indexSource, /Верх сцены/); assert.match(indexSource, /app\.js\?v=10\.2/); assert.match(indexSource, /designer-adapter\.js\?v=10\.2/); assert.match(indexSource, /disguise-plugin-window-size" content="1440,900"/); assert.match(indexSource, /disguise-plugin-window-min-size" content="900,620"/); assert.match(indexSource, /disguise-plugin-window-resizable" content="true"/); assert.match(indexSource, /data-add-type="screen"/); assert.match(indexSource, /id="active-object-strip"/); assert.match(indexSource, /id="live-toggle"/); assert.doesNotMatch(appSource, /Look at Z/); assert.doesNotMatch(appSource, /Наклон X/); assert.doesNotMatch(appSource, /scrub-input|state\.scrub/); assert.doesNotMatch(appSource, /LIVE: макет · отправка отключена/); assert.match(appSource, /addEventListener\("wheel"/);
  ...
  assert.match(indexSource, /id="clear-scene-button"/);
  assert.match(appSource, /path === "geometry\.width" \? "geometry\.height"/); assert.match(appSource, /path === "geometry\.height" && PLANAR_TYPES\.has\(object\.type\) \? "transform\.position\.y"/); assert.match(appSource, /input\.addEventListener\("focus"/); assert.match(appSource, /input\.addEventListener\("pointerup"/); assert.match(appSource, /!state\.sync\.lastSyncAt/); assert.equal(core.formatValue(4), "4"); assert.equal(core.formatValue(1.5), "1,5");
  const heightHarness = createHarness(); const heightScreen = heightHarness.newObject("screen"); heightHarness.state.stage = { centerX: 0, centerZ: 0, floorY: 0, width: 12, depth: 8, height: 1, measureFromStage: true }; heightHarness.setObjectHeight(heightScreen, -3); assert.equal(heightScreen.transform.position.y, -2); assert.equal(heightHarness.objectHeightValue(heightScreen), -3); heightHarness.state.stage.measureFromStage = false; heightHarness.setObjectHeight(heightScreen, -3); assert.equal(heightScreen.transform.position.y, -3);
