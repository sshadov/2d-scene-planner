...
function createHarness(saved, storageKey = "disguise-scene-generator-state-v5") {
  const initialValues = { "#room-width": "20", "#room-depth": "12", "#stage-width": "12", "#stage-depth": "8", "#stage-height": "0.8", "#stage-center-x": "0", "#stage-center-z": "0", "#stage-floor-y": "0.8", "#snap-mode": "grid-1" };
...
function sceneObject(type, pluginId, position = { x: 1, y: 2, z: 3 }) {
  ...
  if (["screen", "projector"].includes(type)) object.media = { resolutionX: 1920, resolutionY: 1080, ...(type === "screen" ? { pixelPitchMm: 2.6 } : {}) };
  return object;
}
...
(async () => {
  const core = createHarness(); assert.equal(core.state.objects.find(object => object.type === "projector")?.transform.position.y, 2.5); assert.equal(core.state.objects.find(object => object.type === "light")?.transform.position.y, 3); assert.equal(core.newObject("camera").transform.position.y, 1.6); const screen = sceneObject("screen", "plugin-screen", { x: 3, y: 0, z: -5 }); const payload = core.objectPayload(screen);
  assert.equal(core.finite("1,5", 9), 1.5); assert.equal(core.finite("1.5", 9), 1.5); assert.equal(core.finite("", 9), 9);
  assert.deepEqual(JSON.parse(JSON.stringify(payload)), { pluginId: "plugin-screen", type: "screen", name: "screen test", transform: { position: { x: 3, y: 0, z: -5 }, rotation: { x: 0, y: 20, z: 0 } }, geometry: { width: 4, height: 2 }, media: { resolutionX: 1920, resolutionY: 1080, pixelPitchMm: 2.6 } });
  ...
  assert.match(adapterSource, /assign\("scale", Vec\(geometry\["width"\], geometry\["height"\], 0\.1\)\)/); assert.match(adapterSource, /assign\("configPosition", position_value\)/); assert.match(adapterSource, /assign\("configRotation", rotation_value\)/); assert.doesNotMatch(adapterSource, /assign\("offset", position_value\)/); assert.match(adapterSource, /assign\("posRelativeOrGlobal"/); assert.match(adapterSource, /assign\("rotRelativeOrGlobal"/); assert.match(adapterSource, /"readback": readback\(obj, kind\)/);
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(migratedV5Harness.state.stage)), { centerX: 3, centerZ: -2, floorY: 1.1, width: 10, depth: 6, height: 0.6 });
  ...
  const schema = JSON.parse(fs.readFileSync(path.join(root, "schemas", "scene.schema.json"), "utf8")); assert.equal(schema.properties.version.const, 6); assert.deepEqual(schema.properties.room.required, ["width", "depth"]); assert.deepEqual(schema.properties.stage.required, ["centerX", "centerZ", "floorY", "width", "depth", "height"]);
  console.log("scene-planner v6 tests: ok");
