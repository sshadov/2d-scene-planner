...
(async () => {
  ...
  assert.match(indexSource, /id="stage-height"/); assert.match(indexSource, /id="measure-from-stage"/); assert.doesNotMatch(indexSource, /Верх сцены/); assert.match(indexSource, /app\.js\?v=9/); assert.match(indexSource, /designer-adapter\.js\?v=9/); assert.doesNotMatch(appSource, /Look at Z/); assert.doesNotMatch(appSource, /Наклон X/);
  ...
  const schema = JSON.parse(fs.readFileSync(path.join(root, "schemas", "scene.schema.json"), "utf8")); assert.equal(schema.properties.version.const, 9); assert.deepEqual(schema.properties.room.required, ["width", "depth"]); assert.deepEqual(schema.properties.stage.required, ["centerX", "centerZ", "floorY", "width", "depth", "height", "measureFromStage"]); assert.equal(core.newObject("projector").lookAt.y, core.state.stage.floorY + core.state.stage.height);
  ...
  assert.match(indexSource, /data-action="mirror-x"/); assert.match(indexSource, /data-action="mirror-z"/);
  console.log("scene-planner v9 tests: ok");
