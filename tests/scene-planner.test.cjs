...
(async () => {
  ...
  assert.match(indexSource, /id="stage-height"/); assert.match(indexSource, /id="measure-from-stage"/); assert.doesNotMatch(indexSource, /Верх сцены/); assert.match(indexSource, /app\.js\?v=7/); assert.match(indexSource, /designer-adapter\.js\?v=7/);
  ...
  const schema = JSON.parse(fs.readFileSync(path.join(root, "schemas", "scene.schema.json"), "utf8")); assert.equal(schema.properties.version.const, 7); assert.deepEqual(schema.properties.room.required, ["width", "depth"]); assert.deepEqual(schema.properties.stage.required, ["centerX", "centerZ", "floorY", "width", "depth", "height", "measureFromStage"]); assert.equal(core.newObject("projector").lookAt.y, core.state.stage.floorY + core.state.stage.height);
  ...
  console.log("scene-planner v7 tests: ok");
