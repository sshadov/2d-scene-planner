...
(async () => {
  ...
  assert.match(indexSource, /id="stage-height"/); assert.match(indexSource, /id="measure-from-stage"/); assert.doesNotMatch(indexSource, /Верх сцены/); assert.match(indexSource, /app\.js\?v=10/); assert.match(indexSource, /designer-adapter\.js\?v=10/); assert.match(indexSource, /data-add-type="screen"/); assert.match(indexSource, /id="active-object-strip"/); assert.match(indexSource, /id="live-toggle"/); assert.doesNotMatch(appSource, /Look at Z/); assert.doesNotMatch(appSource, /Наклон X/); assert.doesNotMatch(appSource, /scrub-input|state\.scrub/); assert.match(appSource, /addEventListener\("wheel"/);
