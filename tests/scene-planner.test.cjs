...
(async () => {
  ...
  assert.equal(core.state.objects.length, 0); assert.equal(core.state.stage.enabled, true); assert.equal(core.newObject("projector").transform.position.y, 2.5); assert.equal(core.newObject("screen").transform.position.y, 0); assert.equal(core.newObject("projector").lookAt.y, 0); assert.equal(core.newObject("screen").media.pixelsPerInch, 10); assert.equal("pixelsPerInch" in core.newObject("surface").media, false);
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(payload)), { pluginId: "plugin-screen", type: "screen", name: "screen test", transform: { position: { x: 3, y: 0, z: -5 }, rotation: { x: 0, y: 20, z: 0 } }, geometry: { width: 4, height: 2 }, media: { resolutionX: 1920, resolutionY: 1200, pixelsPerInch: 10 } }); assert.equal(core.validateReadback(payload, resultFor(payload)), true); assert.throws(() => core.validateReadback(payload, { readback: { transform: { position: { x: 3, y: .0001, z: -5 }, rotation: { x: 0, y: 20, z: 0 } }, geometry: { width: 4, height: 2 } } }), /не пройдена/);
  assert.match(adapterSource, /assign\("offset", Vec\(pos\["x"\], pos\["y"\] \+ geometry\["height"\] \/ 2\.0, pos\["z"\]\)\)/); assert.match(adapterSource, /assign\("scale", Vec\(geometry\["width"\], geometry\["height"\], 0\.1\)\)/); assert.match(adapterSource, /assign\("configPosition", position_value\)/); assert.match(adapterSource, /assign\("configLookAt"/); assert.match(adapterSource, /config_rotation = getattr\(obj, "configRotation"/); assert.match(adapterSource, /stage\.children/); assert.match(adapterSource, /scene_path = "objects\/object\/dsg-scene-cube\.apx"/); assert.match(adapterSource, /LookAtManipulable/); assert.match(adapterSource, /needsMesh/); assert.doesNotMatch(adapterSource, /triangle\.a\s*=/); assert.doesNotMatch(adapterSource, /triangle\.b\s*=/); assert.doesNotMatch(adapterSource, /triangle\.c\s*=/); assert.match(adapterSource, /typeClasses = .*camera: "Camera"/);
  assert.match(indexSource, /id="stage-enabled"/); assert.match(indexSource, /id="stage-center-x"/); assert.match(indexSource, /id="stage-center-z"/); assert.match(indexSource, /id="objects-toggle"/); assert.match(indexSource, /id="clear-scene-button"/); assert.match(indexSource, /app\.js\?v=10\.4/); assert.doesNotMatch(indexSource, /data-add-type=/); assert.doesNotMatch(indexSource, /id="reset-button"/);
  ...
  const heightHarness = createHarness(); const heightScreen = heightHarness.newObject("screen"); heightHarness.state.stage = { enabled: true, centerX: 0, centerZ: 0, floorY: 1, width: 12, depth: 8, height: 1, measureFromStage: true }; heightScreen.transform.position.y = 1; assert.equal(heightHarness.objectHeightValue(heightScreen), 0); heightHarness.setObjectHeight(heightScreen, -3); assert.equal(heightScreen.transform.position.y, -2); assert.equal(heightHarness.objectHeightValue(heightScreen), -3); heightHarness.state.stage.measureFromStage = false; heightHarness.setObjectHeight(heightScreen, -3); assert.equal(heightScreen.transform.position.y, -3);
  const migrated = createHarness({ version: 5, room: { width: 20, depth: 12 }, stage: { centerX: 3, centerZ: -2, floorY: 1.1, width: 10, depth: 6, height: 0.6 }, nextId: 2, objects: [{ id: 1, pluginId: "old-screen", type: "screen", name: "Old", transform: { position: { x: 4, y: 1.5, z: -2 }, rotation: { x: 7, y: 35, z: 9 } }, geometry: { width: 4, height: 2 } }] }); assert.equal(migrated.state.stage.enabled, true); assert.equal(migrated.state.stage.floorY, .5);
  core.state.room = { width: 20, depth: 12 }; core.state.stage = { enabled: true, centerX: 10, centerZ: -5, floorY: .8, width: 12, depth: 8, height: .8, measureFromStage: false }; const frame = { left: 100, top: 50, scale: 10 }; assert.deepEqual(JSON.parse(JSON.stringify(core.toScreen(10, -5, frame))), { x: 300, y: 160 }); assert.deepEqual(JSON.parse(JSON.stringify(core.stageBounds())), { minX: 4, maxX: 16, minZ: -9, maxZ: -1 }); core.state.stage.enabled = false; assert.deepEqual(JSON.parse(JSON.stringify(core.stageBounds())), { minX: -10, maxX: 10, minZ: -6, maxZ: 6 });
  const schema = JSON.parse(fs.readFileSync(path.join(root, "schemas", "scene.schema.json"), "utf8")); assert.equal(schema.properties.stage.properties.enabled.type, "boolean"); assert.ok(schema.$defs.sceneObject.properties.type.enum.includes("designer"));
  core.state.room = { width: 20, depth: 12 }; core.state.stage.enabled = true; core.state.objects = [sceneObject("screen", "hit-screen", { x: 0, y: 0, z: 0 })]; assert.equal(core.hitTest(600, 400).pluginId, "hit-screen"); assert.equal(core.hitTest(600, 410), undefined);
  core.state.objects = [];
  const inspection = { objects: [{ id: "uid-1", type: "projector", path: "objects/projector/dsg-p-1.apx", description: "Front projector", transform: { position: { x: 3, y: 2.5, z: -5 }, rotation: { x: 12, y: 34, z: 56 } }, lookAt: { x: 0, y: 1, z: 0 }, className: "Projector", collection: "projectors", managed: true, pluginId: "p-1" }, { id: "uid-2", type: "designer", path: "objects/prop/manual.apx", description: "Manual prop", transform: { position: { x: 1, y: 0, z: 2 }, rotation: { x: 0, y: 0, z: 0 } }, className: "Prop", collection: "children" }], floorY: 0, floorPosition: { x: 0, y: 0, z: 0 }, roomFloor: { width: 24, depth: 14 }, sceneCube: { designerId: "cube", path: "objects/object/dsg-scene-cube.apx" } };
  await core.importDesignerScene({ inspectScene: async () => inspection }); assert.equal(core.state.objects.length, 2); assert.equal(core.state.objects[0].name, "Front projector"); assert.equal(core.state.objects[0].transform.rotation.z, 56); assert.equal(core.state.objects[1].type, "designer"); assert.equal(core.state.room.width, 24); assert.equal(core.state.sync.sceneCube.designerId, "cube"); assert.equal(core.state.stage.enabled, true);
  const noCubeHarness = createHarness({ version: 5, room: { width: 20, depth: 12 }, stage: { enabled: true, centerX: 0, centerZ: 0, floorY: 0, width: 12, depth: 8, height: 0.8 }, objects: [sceneObject("screen", "local-only")] }); await noCubeHarness.importDesignerScene({ inspectScene: async () => ({ objects: [], floorY: 0, roomFloor: { width: 20, depth: 12 }, sceneCube: null }) }); assert.equal(noCubeHarness.state.objects.length, 0); assert.equal(noCubeHarness.state.stage.enabled, false);
  const current = sceneObject("screen", "managed-screen", { x: 1, y: 1.5, z: 3 }); const previousPayload = core.objectPayload(current); previousPayload.transform.position.y = 0; core.state.objects = [current]; core.state.sync.objects = { "managed-screen": { designerId: "managed-uid", lastExported: "old", payload: previousPayload } }; const managedInspection = { objects: [{ id: "managed-uid", type: "screen", path: "objects/ledscreen/dsg-managed-screen.apx", managed: true }], floorY: 0 }; const diff = await core.makeDiff({ inspectScene: async () => managedInspection }, "update"); assert.deepEqual(JSON.parse(JSON.stringify(diff.update[0].changed)), { transform: { position: { y: 1.5 } } });
