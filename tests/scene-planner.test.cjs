...
(async () => {
  ...
  assert.match(adapterSource, /assign\("offset", Vec\(pos\["x"\], pos\["y"\] \+ geometry\["height"\] \/ 2\.0, pos\["z"\]\)\)/); assert.match(adapterSource, /assign\("scale", Vec\(geometry\["width"\], geometry\["height"\], 0\.1\)\)/); assert.match(adapterSource, /assign\("configPosition", position_value\)/); assert.match(adapterSource, /assign\("configLookAt"/); assert.match(adapterSource, /config_rotation = getattr\(obj, "configRotation"/); assert.match(adapterSource, /stage\.children/); assert.match(adapterSource, /scene_path = "objects\/object\/dsg-scene-cube\.apx"/); assert.match(adapterSource, /typeClasses = .*camera: "Camera"/);
  assert.match(indexSource, /id="stage-enabled"/); assert.match(indexSource, /id="objects-toggle"/); assert.match(indexSource, /id="clear-scene-button"/); assert.match(indexSource, /app\.js\?v=10\.3/); assert.doesNotMatch(indexSource, /data-add-type=/); assert.doesNotMatch(indexSource, /id="reset-button"/);
