...
(async () => {
  ...
  const heightHarness = createHarness(); const heightScreen = heightHarness.newObject("screen"); heightHarness.state.stage = { centerX: 0, centerZ: 0, floorY: 0, width: 12, depth: 8, height: 1, measureFromStage: true }; heightHarness.setObjectHeight(heightScreen, -3); assert.equal(heightScreen.transform.position.y, 1); assert.equal(heightHarness.objectHeightValue(heightScreen), 0); heightHarness.state.stage.measureFromStage = false; heightHarness.setObjectHeight(heightScreen, -3); assert.equal(heightScreen.transform.position.y, 0);
