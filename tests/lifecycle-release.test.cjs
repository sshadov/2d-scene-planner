...
assert.match(cameraCreate, /setattr\(stage, collection_name/);
...
assert.match(remove, /setattr\(stage, collection_name/);
assert.match(remove, /stage\.save\(\)/);
assert.ok(remove.indexOf("setattr(stage, collection_name") < remove.indexOf("resourceManager.remove(path)"));
