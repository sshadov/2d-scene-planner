...
In a hosted Designer window, the fresh inspection is authoritative for object membership; persisted browser objects, selection, history, deletion records, LIVE state, and remembered heights are not restored. Only explicit creates whose Python request is still pending survive an inspection, and an inspected matching resource reuses that Planner identity. Startup performs a best-effort `resourceManager.saveAll()`, then requires a successful import before enabling LIVE. A stopped transport proceeds automatically; running or unknown transport state keeps a blocking responsibility warning visible until accepted. Standalone preview restores its local browser state and never enables Designer integration.
...
The single Diagnostics panel merges Planner, Python API, and LIVE entries into one timestamp-ordered current-session stream and is the sole copy source. Errors remain historical entries and do not gate later operations. Product version `0.22.0` is canonical in `package.json`, the window title, and all static asset cache keys; persisted scene schema remains version `11`. FixtureGroup/DMX Device-list deletion remains intentionally unchanged and is outside this release.
