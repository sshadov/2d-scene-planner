...
Projectors use public `Projector.configPosition`, `Projector.configLookAt`, `Projector.configLookDistance`, and `Projector.configThrowRatio`. Direction is a rounded manual Look At point or an exact `Screen2` binding identified by Designer UID/path. The adapter changes that binding only through `Projector.removeScreen()` and `Projector.addScreen()`. Planner geometry derives look distance, projected width, throw ratio, field of view, and a literal lens-axis roll of `0°` for horizontal surfaces or `90°` for vertical surfaces. During pointer movement LIVE writes only the property being dragged: Position for the Projector body or Look At for its target. After pointerup or a committed field/binding change one HTTP transaction reapplies the complete configuration, writes only `configRotation.z`, saves the Projector and Stage, and applies Designer readback to Planner state. Inherited body `offset/rotation` and continuous LIVE rotation remain outside the contract.
