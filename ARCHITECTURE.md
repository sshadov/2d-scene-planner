...
For screens and surfaces, Enter advances measured values in the order `width -> height -> height above floor/stage`. Numeric focus selects the complete value and integer formatting omits a decimal suffix. Height is a signed world coordinate and may be below the floor or stage; the relative mode displays the signed offset without changing it. The official Live Update transport is a WebSocket with `subscribe`/`valuesChanged`/`set`. It updates writable scalar transforms for mapped objects; explicit Python HTTP synchronization remains available for create, inspect, delete, and full readback operations.
