(() => {
  ...
  const state = {
    ...
    dragging: null, guides: [], pan: { x: 0, y: 0 },
    sync: { objects: {}, lastSyncAt: null },
    lastHeights: {}
  };
  let nextId = 1;
  ...
  let suppressNextEnterKeyup = false;
  ...
  const pendingFieldCommits = new Map();
  const FIELD_COMMIT_DELAY_MS = 500;
  ...
  function normalizeObject(object, index, sourceVersion = VERSION) {
    ...
    if (type === "projector") {
      ...
      normalized.optics = {
        ...
        autoThrowRatio: object.optics?.autoThrowRatio !== false
      };
  ...
  function newObject(type, x = 0, z = 0, rotation = {}) {
    ...
    if (type === "projector") {
      ...
      object.optics = { throwRatio: 1.5, fieldOfView: 40, lookDistance: Math.hypot(x, stageFloorY() - object.transform.position.y, z) || 1.5, autoThrowRatio: true };
  ...
  function bindNumericInput(input, getter, setter, step = .1, defaultValue = undefined, onCommit = undefined, onContinuousChange = undefined, displayGetter = getter) {
    input.dataset.step = step; input.classList.add("wheel-input"); selectInputText(input);
    input.addEventListener("keydown", event => {
      if (event.key !== "ArrowUp" && event.key !== "ArrowDown") return;
      event.preventDefault(); const direction = event.key === "ArrowUp" ? 1 : -1; const multiplier = event.shiftKey ? 10 : 1;
      setter(Number((getter() + direction * step * multiplier).toFixed(3))); input.value = inputDisplayValue(displayGetter(), step); persist(); render(); onContinuousChange?.();
    });
    input.addEventListener("input", () => {
      const parsed = finite(input.value, Number.NaN); if (!Number.isFinite(parsed)) return;
      setter(parsed); persist(); drawScene();
    });
    input.addEventListener("change", () => { const parsed = finite(input.value, Number.NaN); if (!Number.isFinite(parsed)) { input.value = inputDisplayValue(displayGetter(), step); return; } setter(parsed); input.value = inputDisplayValue(displayGetter(), step); persist(); render(); onCommit?.(); });
    input.addEventListener("contextmenu", event => {
      const fallback = typeof defaultValue === "function" ? defaultValue() : defaultValue;
      if (!Number.isFinite(Number(fallback))) return;
      event.preventDefault(); setter(Number(fallback)); input.value = inputDisplayValue(displayGetter(), step); persist(); render(); onCommit?.();
    });
    input.addEventListener("wheel", event => {
      event.preventDefault(); const direction = event.deltaY < 0 ? 1 : -1; const multiplier = event.shiftKey ? 10 : 1;
      setter(Number((getter() + direction * step * multiplier).toFixed(3))); input.value = inputDisplayValue(displayGetter(), step); persist(); render(); onContinuousChange?.();
    }, { passive: false });
  }
  ...
  function setObjectPlanPosition(object, x, z) {
    ...
    if (object.type === "surface") recalculateSurfaceProjectors(object);
  }
  ...
  function projectorProjectedWidth(object) {
    ...
    if (!screen?.geometry) {
      const position = vector(object?.transform?.position); const lookAt = effectiveLookAt(object);
      const distance = Math.max(.001, Math.hypot(lookAt.x - position.x, lookAt.y - position.y, lookAt.z - position.z));
      return Math.max(.1, distance / Math.max(.1, finite(object?.optics?.throwRatio, 1.5)));
    }
    ...
    const projectorAspect = projectorWidthPx / projectorHeightPx;
    return screenHeight > screenWidth ? Math.max(screenHeight, screenWidth * projectorAspect) : Math.max(screenWidth, screenHeight * projectorAspect);
  }
  function projectorProjectedPixelSizeMm(object) {
    const resolutionX = Math.max(1, finite(object?.media?.resolutionX, 1920));
    return Number((projectorProjectedWidth(object) * 1000 / resolutionX).toFixed(3));
  }
  function projectorThrowRatio(object) {
    return object?.optics?.autoThrowRatio !== false ? (projectorAutoThrowRatio(object) ?? Math.max(.1, finite(object?.optics?.throwRatio, 1.5))) : Math.max(.1, finite(object?.optics?.throwRatio, 1.5));
  }
  function projectorAutoThrowRatio(object) {
    if (object?.type !== "projector" || !targetSurface(object)) return null;
    const position = vector(object.transform?.position); const lookAt = effectiveLookAt(object);
    const distance = Math.hypot(lookAt.x - position.x, lookAt.y - position.y, lookAt.z - position.z);
    if (!Number.isFinite(distance) || distance <= 0) return null;
    return Number((distance / projectorProjectedWidth(object)).toFixed(3));
  }
  function projectorRotationZValue(object) {
    const screen = projectorScreen(object);
    if (object?.type !== "projector" || !screen?.geometry) return null;
    return finite(screen.geometry.height) > finite(screen.geometry.width) ? 90 : 0;
  }
  function projectorGeometry(object) {
    const position = vector(object?.transform?.position); const target = effectiveLookAt(object);
    ...
    const direction = { x: Number((delta.x / distance).toFixed(6)), y: Number((delta.y / distance).toFixed(6)), z: Number((delta.z / distance).toFixed(6)) };
    const screen = projectorScreen(object); const storedRatio = Math.max(.1, finite(object?.optics?.throwRatio, 1.5)); const auto = Boolean(screen && object?.optics?.autoThrowRatio !== false);
    const projectedWidth = auto ? projectorProjectedWidth(object) : distance / storedRatio;
    const throwRatio = auto ? Number((distance / projectedWidth).toFixed(3)) : storedRatio;
    const fieldOfView = 2 * Math.atan(projectedWidth / (2 * distance)) * 180 / Math.PI;
    return { direction, distance: Number(distance.toFixed(3)), projectedWidth: Number(projectedWidth.toFixed(3)), throwRatio, fieldOfView: Number(fieldOfView.toFixed(3)), roll: finite(object.projectorRoll, 0) };
  }
  function recalculateProjectorGeometry(object) {
    if (object?.type !== "projector") return null;
    object.optics ||= {};
    const geometry = projectorGeometry(object);
    if (targetSurface(object) && object.optics.autoThrowRatio !== false) object.optics.throwRatio = geometry.throwRatio;
    object.optics.fieldOfView = geometry.fieldOfView;
    object.optics.lookDistance = geometry.distance;
    return geometry;
  }
  function recalculateSurfaceProjectors(surface) {
    if (surface?.type !== "surface") return;
    state.objects.filter(object => object.type === "projector" && object.targetSurfacePluginId === surface.pluginId).forEach(recalculateProjectorGeometry);
  }
  function setProjectorLookDistance(object, value) {
    return false;
  }
  function projectorBeam(object) {
    const geometry = projectorGeometry(object);
    return { distance: geometry.distance, fieldOfView: geometry.fieldOfView, halfWidth: geometry.distance * Math.tan(geometry.fieldOfView * Math.PI / 360) };
  }
  ...
  function rotationHandleGeometry(object, frame = sizing(false)) {
    if (!object || object.type === "projector") return null;
    const centre = toScreen(object.transform.position.x, object.transform.position.z, frame);
    const yaw = finite(object.transform.rotation.y) * Math.PI / 180;
    const objectEdge = PLANAR_TYPES.has(object.type) ? Math.max(8, .1 * frame.scale) : objectRadius(object, frame);
    const lineEnd = objectEdge + 25;
    return { centre, lineX: centre.x + Math.sin(yaw) * objectEdge, lineY: centre.y - Math.cos(yaw) * objectEdge, x: centre.x + Math.sin(yaw) * lineEnd, y: centre.y - Math.cos(yaw) * lineEnd, radius: 7 };
  }
  function drawRotationHandle(object, frame) {
    const handle = rotationHandleGeometry(object, frame); if (!handle) return;
    ctx.save(); ctx.strokeStyle = "rgba(255,255,255,.9)"; ctx.fillStyle = "#17202a"; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.moveTo(handle.lineX, handle.lineY); ctx.lineTo(handle.x, handle.y); ctx.stroke();
    ctx.beginPath(); ctx.arc(handle.x, handle.y, handle.radius, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); ctx.restore();
  }
  function drawScene() { const frame = sizing(); drawGrid(frame); state.objects.filter(object => object.type === "projector").forEach(object => drawProjectorTarget(object, frame)); state.objects.forEach(object => drawObject(object, frame)); drawRotationHandle(selectedObject(), frame); emptyHint.hidden = state.objects.length > 0; document.querySelector("#zoom-reset").textContent = `${Math.round(state.zoom * 100)}%`; document.querySelector("#scene-summary").textContent = `Stage ${formatValue(state.stage.width)} × ${formatValue(state.stage.depth)} m`; }
  function selectedObject() { return state.objects.find(object => object.id === state.selectedId); }
  function selectedHighlightId(object) { return object?.type === "surface" ? object.id : object?.type === "projector" ? targetSurface(object)?.id || null : null; }
  function selectObject(object, allOfType = false) { state.selectedId = object?.id ?? null; state.selectedIds = object ? new Set((allOfType ? state.objects.filter(item => item.type === object.type) : [object]).map(item => item.id)) : new Set(); state.highlightObjectId = selectedHighlightId(object); }
  function toggleObjectSelection(object) {
    ...
    state.selectedIds = ids; state.highlightObjectId = selectedHighlightId(selectedObject());
  }
  ...
  function hitTestRotationHandle(clientX, clientY) { const object = selectedObject(); const handle = rotationHandleGeometry(object); if (!handle) return undefined; const rect = canvas.getBoundingClientRect(); return Math.hypot(clientX - rect.left - handle.x, clientY - rect.top - handle.y) <= handle.radius + 5 ? object : undefined; }
  ...
  function normalizeDiagnostic(source, entry) {
    const raw = entry && typeof entry === "object" ? entry : { message: String(entry || "") };
    const event = String(raw.event || "");
    const level = ["info", "warn", "error"].includes(raw.level) ? raw.level : /error|failed|stale/i.test(event) ? "error" : /warn|retry|reconnect|recover/i.test(event) ? "warn" : "info";
    const subsystem = String(raw.subsystem || (source === "api" ? "Designer API" : source === "live" ? "LIVE" : "Planner"));
    const message = String(raw.message || raw.detail || event || "event");
  ...
  function screenPixelMetrics(object) {
    const widthInches = Math.max(.001, finite(object?.geometry?.width, 0) * 39.37007874);
    const heightInches = Math.max(.001, finite(object?.geometry?.height, 0) * 39.37007874);
    const ppiX = Math.max(.001, finite(object?.media?.resolutionX, 1) / widthInches);
    const ppiY = Math.max(.001, finite(object?.media?.resolutionY, 1) / heightInches);
    const tolerancePPI = .5 / widthInches + .5 / heightInches;
    const valid = Math.abs(ppiX - ppiY) <= tolerancePPI;
    if (!valid) return { valid: false, ppi: "FAIL", pixelPitchMm: "FAIL", ppiX, ppiY, tolerancePPI };
    const ppi = (ppiX + ppiY) / 2;
    return { valid: true, ppi: Number(ppi.toFixed(3)), pixelPitchMm: Number((25.4 / ppi).toFixed(3)), ppiX, ppiY, tolerancePPI };
  }
  function syncScreenMedia(object, changedPath = "") {
    ...
    if (!changedPath.startsWith("media.pixelsPerInch") && changedPath !== "media.pixelPitchMm") return;
    if (changedPath === "media.pixelPitchMm") { media.pixelPitchMm = Math.max(.01, finite(media.pixelPitchMm, 2.6)); media.pixelsPerInch = Number((25.4 / media.pixelPitchMm).toFixed(3)); }
  ...
  function fieldSections(object) {
    ...
    if (object.type === "screen") { const metrics = screenPixelMetrics(object); const mediaFields = [["Resolution X", "media.resolutionX", object.media.resolutionX, 1, "px"], ["Resolution Y", "media.resolutionY", object.media.resolutionY, 1, "px"], ["Pixel density", "media.pixelsPerInch", metrics.ppi, .1, "ppi", { invalid: !metrics.valid }], ["Pixel size", "media.pixelPitchMm", metrics.pixelPitchMm, .001, "mm", { invalid: !metrics.valid }]]; return [{ title: "Size", fields: [["Width", "geometry.width", object.geometry.width, .1, "m"], ["Height", "geometry.height", object.geometry.height, .1, "m"]] }, { title: "Position", fields: [...position, ["Yaw", "transform.rotation.y", object.transform.rotation.y, 1, "°"]] }, { title: "LED data", fields: mediaFields }]; }
    ...
    if (object.type === "projector") { const autoThrowRatio = object.optics?.autoThrowRatio !== false; return [{ title: "Lens position", fields: position }, { title: "Direction", targetSurface: true, fields: [] }, { title: "Resolution", fields: [["X", "media.resolutionX", object.media.resolutionX, 1, "px"], ["Y", "media.resolutionY", object.media.resolutionY, 1, "px"]] }, { title: "Optics", autoThrowRatio: true, fields: [["Throw ratio", "optics.throwRatio", object.optics.throwRatio, .01, "x", { readOnly: autoThrowRatio }], ["Field of view", "optics.fieldOfView", object.optics.fieldOfView, .1, "°", { readOnly: true }], ["Look distance", "optics.lookDistance", object.optics.lookDistance, .1, "m", { readOnly: true }], ["Projected pixel size", "derived.projectedPixelSizeMm", projectorProjectedPixelSizeMm(object), .001, "mm", { readOnly: true }]] }]; }
  ...
  function getFieldValue(object, path) { return path === "transform.position.y" ? objectHeightValue(object) : path === "projector.yaw" ? projectorYaw(object) : path === "derived.projectedPixelSizeMm" ? projectorProjectedPixelSizeMm(object) : getPath(object, path); }
  function fieldDisplayValue(object, path) { if (object.type === "screen" && path === "media.pixelsPerInch") return screenPixelMetrics(object).ppi; if (object.type === "screen" && path === "media.pixelPitchMm") return screenPixelMetrics(object).pixelPitchMm; return getFieldValue(object, path); }
  function setPath(object, path, value) {
    if (path === "projector.yaw") { setProjectorYaw(object, value); return; }
    if (object.type === "projector" && path === "optics.lookDistance") return;
    ...
    if (PLANAR_TYPES.has(object.type)) { object.transform.rotation.x = 0; object.transform.rotation.z = 0; } if (object.type === "screen" && (path.startsWith("media.") || path.startsWith("geometry."))) syncScreenMedia(object, path); if (object.type === "surface" && (path.startsWith("geometry.") || path.startsWith("transform."))) recalculateSurfaceProjectors(object); if (object.type === "projector" && path.startsWith("media.resolution") && targetSurface(object)) { recalculateProjectorGeometry(object); scheduleProjectorGeometry(object, "resolution"); }
  }
  function fieldCommitKey(objectOrPluginId, path) { return `${typeof objectOrPluginId === "string" ? objectOrPluginId : objectOrPluginId?.pluginId || ""}|${path || ""}`; }
  function cancelPendingFieldCommits(objectOrPluginId, path = null) {
    const pluginId = typeof objectOrPluginId === "string" ? objectOrPluginId : objectOrPluginId?.pluginId;
    if (!pluginId) return;
    for (const [key, timer] of pendingFieldCommits) {
      if (path ? key !== fieldCommitKey(pluginId, path) : !key.startsWith(`${pluginId}|`)) continue;
      clearTimeout(timer); pendingFieldCommits.delete(key);
    }
  }
  function scheduleFieldCommit(object, path) {
    if (!object?.pluginId || !path) return;
    const key = fieldCommitKey(object, path);
    cancelPendingFieldCommits(object, path);
    pendingFieldCommits.set(key, setTimeout(() => {
      pendingFieldCommits.delete(key);
      const current = state.objects.find(candidate => candidate.pluginId === object.pluginId);
      if (current) void commitFieldChange(current, path);
    }, FIELD_COMMIT_DELAY_MS));
  }
  function commitFieldChangeNow(object, path) { cancelPendingFieldCommits(object, path); return commitFieldChange(object, path); }
  function queueFieldFocus(object, path) { pendingFocusPath = path || null; pendingFocusPluginId = path ? object?.pluginId || null : null; }
  ...
  function suppressDesignerEnter(event) { if (event.key !== "Enter") return; event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation?.(); }
  window.addEventListener("keyup", event => { if (!suppressNextEnterKeyup || event.key !== "Enter") return; suppressNextEnterKeyup = false; suppressDesignerEnter(event); }, true);
  function makeObjectField(object, definition) { const [labelText, path, value, step, unit, options = {}] = definition; const label = element("label", "object-field"); label.append(element("span", "object-field-label", labelText)); const shell = element("span", "input-shell"); const input = document.createElement("input"); input.type = "text"; input.inputMode = "decimal"; input.value = inputDisplayValue(value, step); input.dataset.field = path; input.dataset.pluginId = object.pluginId; input.setAttribute("aria-label", `${object.name}: ${labelText}`); if (options.readOnly) { input.readOnly = true; input.classList.add("readonly"); } if (options.invalid) { input.classList.add("derived-invalid"); input.setAttribute("aria-invalid", "true"); } activeFieldRefs.set(path, input); shell.append(input, element("i", "", unit)); label.append(shell); if (options.readOnly) return label; bindNumericInput(input, () => getFieldValue(object, path), next => setPath(object, path, next), step, () => defaultFieldValue(object, path), () => { void commitFieldChangeNow(object, path); }, () => { scheduleLiveSync(0); scheduleFieldCommit(object, path); }, () => fieldDisplayValue(object, path)); input.addEventListener("keydown", event => { if (event.key !== "Enter") return; suppressNextEnterKeyup = true; suppressDesignerEnter(event); const parsed = finite(input.value, Number.NaN); if (Number.isFinite(parsed)) setPath(object, path, parsed); persist(); const nextPath = nextDimensionField(object, path); if (nextPath) { queueFieldFocus(object, nextPath); refreshActiveValues(); requestAnimationFrame(() => focusActiveField(nextPath)); setTimeout(() => focusActiveField(nextPath), 20); } else { input.value = inputDisplayValue(fieldDisplayValue(object, path), step); input.blur?.(); } }); input.addEventListener("keypress", suppressDesignerEnter); return label; }
  ...
  async function commitObjectName(object, input) {
    ...
    object.name = nextName; persist(false); render();
    ...
    try {
      ...
      plannerLog("error", "Rename", `Rename failed: ${error.message || error}`, { objectName: object.name, phase: "rename" });
      persist(false); render();
  ...
  function refreshActiveValues() { const object = selectedObject(); if (!object) return; document.querySelectorAll("#active-object-strip input[data-field]").forEach(input => { if (document.activeElement === input) return; const step = finite(input.dataset.step, .1); const value = fieldDisplayValue(object, input.dataset.field); input.value = inputDisplayValue(value, step); if (value === "FAIL") { input.classList.add("derived-invalid"); input.setAttribute("aria-invalid", "true"); } else { input.classList.remove("derived-invalid"); input.removeAttribute?.("aria-invalid"); } }); }
  function renderStatus() { const statuses = state.objects.map(objectSyncStatus); document.querySelector("#status-synced").textContent = statuses.filter(status => status === "synced").length; document.querySelector("#status-changed").textContent = statuses.filter(status => status === "changed").length; renderInfo(); }
  ...
  function updateProjectorTargetPlacement(point) {
    ...
    object.lookAt.x = Number(point.x.toFixed(3));
    object.lookAt.z = Number(point.z.toFixed(3));
  ...
  function commitProjectorTargetPlacement(point) {
    ...
    queueFieldFocus(object, "transform.position.y");
    persist();
    render();
  ...
  function pasteCopiedObjects() { if (!clipboardObjects.length) return false; const pasted = clipboardObjects.map(source => { const copy = clone(source); copy.id = nextId++; copy.pluginId = makeId(); copy.name = nextObjectName(copy.type); delete copy.designer; copy.transform.position.x = Number((copy.transform.position.x + .5).toFixed(3)); logPlannerAction("duplicate", copy, { phase: "local", method: "paste", sourcePluginId: source.pluginId }); return copy; }); state.objects.push(...pasted); selectObject(pasted[pasted.length - 1]); persist(); render(); pasted.forEach(object => { void createDesignerObject(object); }); return true; }
  function rotateObject90(id) { const object = state.objects.find(item => item.id === id); if (!object || object.type === "projector") return; object.transform.rotation.y = normalizeYaw(object.transform.rotation.y + 90); persist(); render(); }
  function alignObjectToStage(object, action) {
    if (!object) return false;
    const bounds = stageBounds();
    const yaw = finite(object.transform?.rotation?.y) * Math.PI / 180;
    const halfWidth = PLANAR_TYPES.has(object.type) ? finite(object.geometry?.width) / 2 : 0;
    const halfThickness = PLANAR_TYPES.has(object.type) ? .05 : 0;
    const extentX = Math.abs(Math.cos(yaw)) * halfWidth + Math.abs(Math.sin(yaw)) * halfThickness;
    const extentZ = Math.abs(Math.sin(yaw)) * halfWidth + Math.abs(Math.cos(yaw)) * halfThickness;
    const values = { "align-left": ["x", bounds.minX + extentX], "align-center-x": ["x", 0], "align-right": ["x", bounds.maxX - extentX], "align-top": ["z", bounds.maxZ - extentZ], "align-center-z": ["z", 0], "align-bottom": ["z", bounds.minZ + extentZ] };
    const target = values[action]; if (!target) return false;
    const [axis, value] = target;
    setObjectPlanPosition(object, axis === "x" ? Number(value.toFixed(3)) : object.transform.position.x, axis === "z" ? Number(value.toFixed(3)) : object.transform.position.z);
    persist(); render(); projectorsAffectedBy([object]).forEach(projector => scheduleProjectorGeometry(projector, "align"));
    return true;
  }
  ...
  async function deleteObject(id, options = {}) {
    ...
    const removed = state.objects[index]; let record = state.sync.objects?.[removed.pluginId];
    cancelPendingFieldCommits(removed);
    ...
    const deleteFromDeviceList = removed.type !== "dmxLight" && Boolean(options.deleteFromDeviceList);
    ...
    persist(); render();
    if (!deleteFromDesigner) return;
    if (!record?.designerId && pendingDesignerCreates.has(removed.pluginId)) {
      await pendingDesignerCreates.get(removed.pluginId);
      record = state.sync.objects?.[removed.pluginId];
    }
    ...
    try {
      ...
      let result;
      if (record.owned) {
        result = await adapter.deleteManagedObjects([{ id: record.designerId, path: record.path, owned: true, ownedPaths: Array.isArray(record.ownedPaths) ? record.ownedPaths : [], removeResource: deleteFromDeviceList }]);
      } else {
      ...
      if (deviceListError) plannerLog("error", removed.type === "dmxLight" ? "DMX Light" : "Designer delete", `Device list cleanup failed: ${deviceListError}`, { objectName: removed.name, phase: "device-list" });
      ...
      plannerLog("error", "Designer delete", `Delete failed: ${error.message || error}`, { objectName: removed.name, phase: "delete" }); persist(false); renderObjectGroups();
    }
  }
  ...
  function objectPayload(object) { const payload = { pluginId: object.pluginId, type: object.type, name: object.name, transform: clone(object.transform) }; if (object.lookAt) payload.lookAt = clone(effectiveLookAt(object)); if (object.geometry) payload.geometry = clone(object.geometry); if (["screen", "dmxScreen", "surface", "projector"].includes(object.type) && object.media) payload.media = clone(object.media); if (object.optics) payload.optics = { throwRatio: object.optics.throwRatio, fieldOfView: object.optics.fieldOfView, lookDistance: object.optics.lookDistance }; if (object.type === "projector") { const surface = targetSurface(object); const record = surface ? state.sync.objects?.[surface.pluginId] : null; payload.targetSurface = surface ? { pluginId: surface.pluginId, designerId: record?.designerId || null, path: record?.path || null, name: surface.name } : null; payload.projectorRoll = finite(object.projectorRoll, 0); } return payload; }
  ...
  function persistCreatedRecord(records, object, payload, result, options = {}) {
    ...
    const degradedReasons = ownershipError ? [`Ownership metadata: ${ownershipError.message || ownershipError}`] : [];
    const record = {
      ...
      owned: Boolean(options.owned), ownedPaths, creationState: degradedReasons.length ? "degraded" : "created", degradedReasons
    };
    ...
    const operation = (async () => {
      ...
      try {
        ...
        try { validateReadback(payload, result); record.readbackValid = true; }
          ...
          record.readbackValid = false; record.lastExported = null; record.creationState = "degraded";
          record.degradedReasons.push(`Readback: ${validationError.message || validationError}`);
        }
        if (record.creationState === "degraded") plannerLog("warn", "Designer create", `Created with degraded metadata: ${record.degradedReasons.join("; ")}`, { objectName: object.name, phase: "create-degraded" });
        state.sync.lastSyncAt = new Date().toISOString();
        persist(false); render();
        return record;
      } catch (error) {
        plannerLog("error", "Designer create", `Create failed: ${error.message || error}`, { objectName: object.name, phase: "create" });
  ...
  function flushProjectorProjection(object, source = "planner", final = false, generation = projectorGenerations.get(object?.pluginId)) {
    const pluginId = object?.pluginId;
    const pending = projectorPending.get(pluginId);
    if (!pluginId || !pending || generation !== pending.generation || generation !== projectorGenerations.get(pluginId)) return false;
    const current = state.objects.find(candidate => candidate.pluginId === pluginId);
    if (!current || current.type !== "projector") return false;
    ...
    if (final) {
      pending.finalTimer = null;
      if (generation === projectorGenerations.get(pluginId) && targetSurface(current)) void finalizeProjectorRotation(current, source, generation);
    }
    return Boolean(sent);
  }
  function queueProjectorProjection(object, source = "planner") {
    if (object?.type !== "projector" || object.manualLookAtPendingUnbind) return 0;
    const pluginId = object.pluginId; const generation = (projectorGenerations.get(pluginId) || 0) + 1;
    projectorGenerations.set(pluginId, generation);
    const previous = projectorPending.get(pluginId);
    if (previous?.cadenceTimer) clearTimeout(previous.cadenceTimer);
    if (previous?.finalTimer) clearTimeout(previous.finalTimer);
    const pending = { lastSentAt: previous?.lastSentAt || 0, cadenceTimer: null, finalTimer: null, generation };
    projectorPending.set(pluginId, pending);
    const delay = Math.max(0, PROJECTOR_LIVE_INTERVAL_MS - (Date.now() - pending.lastSentAt));
    pending.cadenceTimer = setTimeout(() => flushProjectorProjection(object, source, false, generation), delay);
    pending.finalTimer = setTimeout(() => flushProjectorProjection(object, source, true, generation), PROJECTOR_FINAL_DELAY_MS);
    return generation;
  }
  ...
  function commitProjectorBinding(object) {
    ...
    return queueConfigurationCommit(object, async isLatest => {
      const surface = targetSurface(object);
      if (surface && !state.sync.objects?.[surface.pluginId]?.designerId) {
        const created = await createDesignerObject(surface);
        if (!created?.designerId) throw new Error("Bound Surface could not be created in Designer");
      }
      ...
      const targetSurfaceValue = surface ? { pluginId: surface.pluginId, designerId: surfaceRecord?.designerId || null, path: surfaceRecord?.path || null, name: surface.name } : null;
      try {
        ...
        persist(false);
        queueProjectorProjection(object, "surface-binding");
        return result;
      } catch (error) {
        if (isLatest()) plannerLog("error", "Projector", `Surface binding failed: ${error.message || error}`, { objectName: object.name, phase: "surface-binding" });
        return null;
      }
    });
  }
  function cancelProjectorWork(pluginId) {
    const pending = projectorPending.get(pluginId);
    if (pending?.cadenceTimer) clearTimeout(pending.cadenceTimer);
    if (pending?.finalTimer) clearTimeout(pending.finalTimer);
    projectorPending.delete(pluginId);
    projectorGenerations.set(pluginId, (projectorGenerations.get(pluginId) || 0) + 1);
  }
  async function finalizeProjectorRotation(object, source = "final", generation = projectorGenerations.get(object?.pluginId)) {
    const value = projectorRotationZValue(object);
    if (value === null || generation !== projectorGenerations.get(object?.pluginId)) return null;
    const record = state.sync.objects?.[object.pluginId]; const adapter = getAdapter();
    ...
    try {
      ...
      const result = adapter.updateProjectorRotationZ ? await adapter.updateProjectorRotationZ(record.designerId, value, record.path) : null;
      if (generation !== projectorGenerations.get(object.pluginId)) return null;
      object.projectorRoll = Number.isFinite(Number(result?.readback?.projectorRoll)) ? Number(result.readback.projectorRoll) : value;
      persist(false); refreshActiveValues(); drawScene(); return result;
    } catch (error) { plannerLog("warn", "Projector", `Rotation Z failed: ${error.message || error}`, { objectName: object.name, phase: "rotation", source }); return null; }
  }
  function handleProjectorLookDistance(object, value, options = {}) {
    object.optics ||= {};
    object.optics.lookDistance = Math.max(.1, finite(value, object.optics.lookDistance));
  ...
  function commitSurfaceConfiguration(object) {
    ...
    return queueConfigurationCommit(object, async isLatest => {
      const record = state.sync.objects?.[object.pluginId]; const adapter = getAdapter();
      if (!record?.designerId || !adapter?.updateObject) return null;
      const changed = { transform: payload.transform, geometry: payload.geometry, media: payload.media };
      try {
        ...
        persist(false); renderStatus(); return result;
      } catch (error) {
        if (isLatest()) {
          plannerLog("error", "Surface", `Surface update failed: ${error.message || error}`, { objectName: object.name, phase: "surface-update" }); persist(false); renderStatus();
  ...
  async function commitFieldChange(object, path) {
    ...
    if (object.type === "projector" && (path.startsWith("transform.") || path.startsWith("lookAt."))) { queueProjectorProjection(object, "numeric"); return Promise.resolve([]); }
    if (object.type === "projector" && path === "optics.throwRatio" && object.optics?.autoThrowRatio === false) { queueProjectorProjection(object, "numeric-throw-ratio"); return Promise.resolve([]); }
    if (object.type === "projector" && path.startsWith("optics.")) return Promise.resolve([]);
    if (object.type === "projector" && path.startsWith("media.")) {
      const record = state.sync.objects?.[object.pluginId]; const adapter = getAdapter();
      if (!record?.designerId || !adapter?.updateObject) return [];
      try {
        const changed = { media: clone(object.media) };
        const result = await adapter.updateObject(record.designerId, changed, record.path, "projector");
        validateReadback(objectPayload(object), result);
        record.payload = objectPayload(object); record.lastExported = canonical(record.payload); record.readbackValid = true;
        if (result?.path) record.path = result.path;
        persist(false); renderStatus(); return result;
      } catch (error) {
        plannerLog("error", "Projector", `Resolution update failed: ${error.message || error}`, { objectName: object.name, phase: "resolution-update" });
        persist(false); renderStatus(); return null;
      }
    }
    if (object.type === "surface" && (path.startsWith("transform.") || path.startsWith("geometry.") || path.startsWith("media."))) {
      const result = await commitSurfaceConfiguration(object);
      if (result) projectorsAffectedBy([object]).forEach(projector => queueProjectorProjection(projector, "surface"));
      return [];
    }
    if (["screen", "dmxScreen"].includes(object.type) && (path.startsWith("media.") || path.startsWith("transform.") || path.startsWith("geometry."))) {
      const record = state.sync.objects?.[object.pluginId]; const adapter = getAdapter();
      if (!record?.designerId || !adapter?.updateObject) return [];
      const changed = path.startsWith("media.") ? { media: clone(object.media) } : { transform: clone(object.transform), geometry: clone(object.geometry), ...(object.media ? { media: clone(object.media) } : {}) };
      try {
        const result = await adapter.updateObject(record.designerId, changed, record.path, object.type);
        record.payload = objectPayload(object); record.lastExported = canonical(record.payload); record.readbackValid = true;
        if (result?.path) record.path = result.path;
        persist(false); renderStatus(); return result;
      } catch (error) {
        plannerLog("error", object.type === "dmxScreen" ? "DMX Screen" : "LED Screen", `Screen update failed: ${error.message || error}`, { objectName: object.name, phase: "screen-update" });
        persist(false); renderStatus(); return null;
      }
    }
  ...
  async function syncToDesigner(diff) {
    ...
    for (const item of pendingCreates) {
      ...
      try {
        ...
        catch (validationError) { record.readbackValid = false; record.lastExported = null; record.creationState = "degraded"; record.degradedReasons = [...(record.degradedReasons || []), `Readback: ${validationError.message || validationError}`]; persist(false); plannerLog("warn", "Designer create", `Created with degraded readback: ${validationError.message || validationError}`, { objectName: item.object.name, phase: "create-degraded" }); }
      } catch (error) {
    ...
    state.sync.lastSyncAt = new Date().toISOString(); persist(false);
  }
  ...
  function applyLiveValue(change) {
    ...
    if (object.type === "surface" && !change.initial && (change.field.startsWith("transform.") || change.field.startsWith("geometry."))) projectorsAffectedBy([object]).forEach(projector => queueProjectorProjection(projector, "designer-surface"));
    const record = state.sync.objects?.[object.pluginId];
  ...
  function scheduleLiveSceneImport() {
    ...
    liveSceneImportTimer = setTimeout(async () => {
      ...
      const adapter = getAdapter();
      if (!adapter) return;
      try { const inspection = await importDesignerScene(adapter, { preserveLocal: true }); if (inspection?.complete !== false) document.querySelector("#adapter-status").textContent = "LIVE: Designer object list updated"; }
      catch (error) { plannerLog("error", "LIVE", error.message || String(error), { phase: "scene-import" }); persist(false); liveStatus({ status: "error", detail: error.message || String(error) }); }
    }, 120);
  }
  async function runLiveSync() {
    ...
    try {
      ...
      const sent = adapter.liveSync(boundObjects.map(object => ({ payload: livePayload(object), record: state.sync.objects[object.pluginId], subscribeOnly: object.type === "projector" })));
      ...
      plannerLog("error", "LIVE", error.message || String(error), { phase: "send" }); persist(false); liveStatus({ status: "error", detail: error.message || String(error) });
    } finally {
  ...
  async function startLive() {
    ...
    if (liveState && liveState.socket !== "open" && !liveState.wanted) throw new Error("Live Update WebSocket did not open");
  ...
  function importedObject(item, index) {
    ...
    if (knownType === "projector") {
      ...
      object.optics = { throwRatio: Math.max(.1, finite(item.optics?.throwRatio, existing?.optics?.throwRatio || 1.5)), fieldOfView: Math.max(.1, finite(item.optics?.fieldOfView, existing?.optics?.fieldOfView || 40)), lookDistance: Math.max(.1, finite(item.optics?.lookDistance, existing?.optics?.lookDistance || Math.hypot(object.lookAt.x - object.transform.position.x, object.lookAt.y - object.transform.position.y, object.lookAt.z - object.transform.position.z))), autoThrowRatio: existing?.optics?.autoThrowRatio !== false };
  ...
  async function importDesignerScene(adapter, options = {}) {
    ...
    if (inspection?.complete === false) {
      ...
      plannerLog("error", "Designer import", `Incomplete scene inspection${messages.length ? `: ${messages.join("; ")}` : ""}`, { phase: options.preserveLocal ? "live-inspection" : "startup-inspection" });
    ...
    state.highlightObjectId = selectedHighlightId(selected);
    queueFieldFocus(selected && selected.pluginId === previousSelectedPluginId ? selected : null, selected && selected.pluginId === previousSelectedPluginId ? activeFieldPath : null);
    ...
    return { ...inspection, complete: true };
  }
  ...
  document.querySelector("#json-button")?.addEventListener("click", copySceneJson);
  infoButton?.addEventListener("click", () => { const nextHidden = !infoPopover.hidden; infoPopover.hidden = nextHidden; infoButton.setAttribute("aria-expanded", String(!nextHidden)); renderInfo(); });
  document.querySelector("#zoom-in")?.addEventListener("click", () => { state.zoom = clamp(state.zoom + .1, ZOOM_MIN, ZOOM_MAX); drawScene(); }); document.querySelector("#zoom-out")?.addEventListener("click", () => { state.zoom = clamp(state.zoom - .1, ZOOM_MIN, ZOOM_MAX); drawScene(); });
  ...
  canvas.addEventListener("pointerdown", event => {
    ...
    const rotationOwner = hitTestRotationHandle(event.clientX, event.clientY);
    const targetOwner = rotationOwner ? null : hitTestProjectorTarget(event.clientX, event.clientY);
    let object = rotationOwner || targetOwner || hitTest(event.clientX, event.clientY);
    if (!object) {
    ...
    if (!object) return;
    if (rotationOwner) {
      const centre = toScreen(object.transform.position.x, object.transform.position.z, sizing(false));
      const pointerAngle = Math.atan2(event.clientX - rect.left - centre.x, -(event.clientY - rect.top - centre.y)) * 180 / Math.PI;
      state.dragging = { kind: "rotate", id: object.id, pending: true, startClientX: event.clientX, startClientY: event.clientY, startYaw: finite(object.transform.rotation.y), startPointerAngle: pointerAngle };
    } else if (targetOwner) {
  ...
  canvas.addEventListener("pointermove", event => {
    ...
    if (state.dragging.kind === "rotate") {
      const centre = toScreen(object.transform.position.x, object.transform.position.z, sizing(false));
      const pointerAngle = Math.atan2(event.clientX - rect.left - centre.x, -(event.clientY - rect.top - centre.y)) * 180 / Math.PI;
      const delta = (pointerAngle - state.dragging.startPointerAngle + 540) % 360 - 180;
      object.transform.rotation.y = normalizeYaw(state.dragging.startYaw + delta);
    } else if (state.dragging.kind === "group") {
      ...
      const dx = targetX - primaryStart.x;
      const dz = targetZ - primaryStart.z;
      ...
      const x = Number(snapCoordinate("x", point.x + state.dragging.offsetX, object).toFixed(3));
      const z = Number(snapCoordinate("z", point.z + state.dragging.offsetZ, object).toFixed(3));
      setObjectPlanPosition(object, x, z);
    }
    drawScene();
    refreshActiveValues();
    scheduleLiveSync(0);
    const movedObjects = state.dragging.kind === "group" ? state.dragging.positions.map(position => state.objects.find(candidate => candidate.id === position.id)).filter(Boolean) : [object];
    projectorsAffectedBy(movedObjects).forEach(projector => scheduleProjectorGeometry(projector, "drag", 40));
  });
  function clearDragState(event, commit = true) { const pointerId = event?.pointerId; const drag = state.dragging; const moved = Boolean(drag && !drag.pending && drag.kind !== "pan"); const affected = moved ? (drag.kind === "group" ? drag.positions.map(position => state.objects.find(object => object.id === position.id)).filter(Boolean) : [state.objects.find(object => object.id === drag.id)].filter(Boolean)) : []; if (commit && moved && liveSyncTimer) { clearTimeout(liveSyncTimer); liveSyncTimer = null; void runLiveSync(); } state.dragging = null; state.guides = []; if (commit) { persist(!moved); if (moved) projectorsAffectedBy(affected).forEach(object => scheduleProjectorGeometry(object, "pointerup", 0)); } render(); if (pointerId !== undefined && canvas.hasPointerCapture?.(pointerId)) canvas.releasePointerCapture(pointerId); }
  ...
  async function finishStartup(adapter) {
    ...
    try { const inspection = await importDesignerScene(adapter); if (inspection?.complete === false) throw new Error("Designer scene inspection was incomplete"); adapterStatus.textContent = "Designer scene imported"; }
    catch (error) {
